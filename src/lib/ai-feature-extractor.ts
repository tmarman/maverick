  ...
  private async gatherRepositoryContext(request: FeatureExtractionRequest): Promise<string[]> {
    ...
    for (const pattern of includePatterns) {
      try {
        ...
        console.warn(`Error gathering files for pattern ${pattern}:`, error)
      }
    }
    ...
    try {
      ...
      const response = await this.aiProvider.generateResponse({
        messages: [
          {
            role: 'system',
            content: `You are an expert software architect analyzing a codebase for features and improvements. 
